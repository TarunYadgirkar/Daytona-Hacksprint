...
test("recorded stream emits the complete gate contract without external work", async () => {
  const previous = process.env.SAFESHIP_RECORDED_DELAY_MS;
  process.env.SAFESHIP_RECORDED_DELAY_MS = "0";
  const events: GateEvent[] = [];
  try {
    await streamRecordedRun(RECORDED_RUNS[0]!, (event) => events.push(event));
  } finally {
    if (previous === undefined) delete process.env.SAFESHIP_RECORDED_DELAY_MS;
    else process.env.SAFESHIP_RECORDED_DELAY_MS = previous;
  }
